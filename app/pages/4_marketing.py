...
st.markdown(
    f'<div class="section-header">Channel performance · last {days} days</div>',
    unsafe_allow_html=True,
)
...
if ch_sum.empty:
    ...
    ch_display["ROAS"]              = ch_display["roas"].apply(
        lambda v: _fmt_x(v) if pd.notna(v) else "N/A"
    )
    ch_display["Conversion rate"]   = ch_display["conversion_rate"].apply(
...
with chart_l:
    ...
    st.markdown(
        '<div class="chart-title">Spend vs revenue attributed</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        f'<div class="chart-sub">By channel · last {days} days</div>',
        unsafe_allow_html=True,
    )
    ...
    if not ch_sum.empty:
        ...
        fig.add_trace(go.Bar(
            y=ch_sorted["channel"],
            x=ch_sorted["revenue_attributed"],
            name="Revenue",
            orientation="h",
            marker_color="#1D4ED8",
    ...
    _ai_chart_insight(
        ...
        context=(
            f"Horizontal grouped bar chart comparing spend vs revenue attributed "
...
with chart_r:
    st.markdown('<div class="chart-card">', unsafe_allow_html=True)
    st.markdown(
        '<div class="chart-title">ROAS by channel</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        f'<div class="chart-sub">Last {days} days · 1.0x = break even</div>',
        unsafe_allow_html=True,
    )
    ...
    _ai_chart_insight(
        key="marketing_roas",
        context=(
            f"Horizontal bar chart of ROAS by channel over the last {days} days. "
            f"Bars below 1.5x are red (underperforming), 1.5-3x are light blue, "
            f"above 3x are dark blue (strong). Break-even line at 1.0x. "
            + (
                f"Best channel: {roas_df.iloc[-1]['channel']} at "
                f"{roas_df.iloc[-1]['roas']:.2f}x. "
                f"Worst: {roas_df.iloc[0]['channel']} at "
                f"{roas_df.iloc[0]['roas']:.2f}x."
                if not ch_sum.empty and not roas_df.empty else ""
            )
        ),
        ai_on=ai_on,
        has_key=has_key,
...
st.markdown(
    f'<div class="chart-sub">Last {days} days · stacked</div>',
    unsafe_allow_html=True,
)
...
_ai_chart_insight(
    ...
    context=(
        f"Stacked bar chart of daily spend by channel over the last {days} days. "
