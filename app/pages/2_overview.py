...
from analytics.metrics import load_channel_metrics, compute_channel_summary
...
def _ai_chart_insight(key: str, context: str, ai_on: bool, has_key: bool):
    with st.expander("AI insight for this chart"):
        ...
        if not ai_on:
            st.markdown(
                '<div class="ai-placeholder">Turn on "Show AI insights on charts" in the sidebar, then click Generate.</div>',
                unsafe_allow_html=True,
            )
            return
        btn_key = f"btn_{key}"
...
with k1:
    _kpi_card(
        "Revenue",
        _fmt_currency(rev.get("recent_avg")),
        _fmt_pct(rev.get("pct_change")),
        _delta_class(rev.get("pct_change")),
...
with k2:
    _kpi_card(
        ...
        _fmt_pct(ord_.get("pct_change")),
        _delta_class(ord_.get("pct_change")),
        hint=f"vs {ord_.get('prior_avg', 0):,.0f} prior" if ord_.get("prior_avg") else "",
    )
with k3:
    cr_recent = cr.get("recent_avg")
    cr_prior  = cr.get("prior_avg")
    _kpi_card(
        "Conversion rate",
        f"{cr_recent * 100:.2f}%" if cr_recent else "N/A",
        _fmt_pct(cr.get("pct_change")),
        _delta_class(cr.get("pct_change")),
        hint=f"vs {cr_prior * 100:.2f}% prior" if cr_prior else "",
    )
with k4:
    _kpi_card(
        "CAC",
        _fmt_currency(cac.get("recent_avg")),
        _fmt_pct(cac.get("pct_change")),
        _delta_class(cac.get("pct_change"), inverse=True),
...
with k5:
    _kpi_card(
        "ROAS",
        _fmt_x(roas.get("recent_avg")),
        _fmt_pct(roas.get("pct_change")),
        _delta_class(roas.get("pct_change")),
        hint=f"vs {_fmt_x(roas.get('prior_avg'))} prior",
    )
with k6:
    _kpi_card(
        "AOV",
        _fmt_currency(aov.get("recent_avg")),
        _fmt_pct(aov.get("pct_change")),
        _delta_class(aov.get("pct_change")),
...
with k7:
    ...
    _kpi_card(
        "Total spend",
        _fmt_currency(spend.get("recent_avg")),
        _fmt_pct(spend.get("pct_change")),
        _delta_class(spend.get("pct_change"), inverse=True),
...
with ch_left:
    ...
    st.markdown(
        f'<div class="chart-sub">Last {days} days with 7-day rolling average</div>',
        unsafe_allow_html=True,
    )
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            showgrid=False, tickfont=dict(size=11), tickformat="%b %d",
        ),
        yaxis=dict(
    ...
    _ai_chart_insight(
        ...
        context=(
            f"Daily revenue bar chart for the last {days} days with a 7-day rolling "
            f"average overlay. Recent avg: {_fmt_currency(rev.get('recent_avg'))}, "
...
with ch_right:
    ...
    st.markdown(
        f'<div class="chart-sub">Last {days} days · all metrics</div>',
        unsafe_allow_html=True,
    )

    if recent_anom.empty:
        st.markdown(
            "<div style='padding:1rem 0; color:#9CA3AF; font-size:0.8rem;'>"
            "No anomalies detected in this window.</div>",
            unsafe_allow_html=True,
        )
    else:
        display_anom = recent_anom.head(6)
        for _, row in display_anom.iterrows():
            ...
            <div style="display:flex; align-items:center; justify-content:space-between;
                ...
                <div>
                    <div style="font-size:0.78rem; font-weight:600; color:#111827;">{metric.replace('_',' ').title()}</div>
...
st.markdown(
    f'<div class="chart-sub">Last {days} days · dual axis</div>',
    unsafe_allow_html=True,
)
...
fig2.add_trace(go.Scatter(
    x=recent_df["date"],
    y=(recent_df["conversion_rate"] * 100).round(2),
...
fig2.update_layout(
    ...
    yaxis=dict(
        showgrid=True, gridcolor="#F3F4F6",
        tickfont=dict(size=11), ticksuffix="%", title="Conv rate",
        title_font=dict(size=11),
    ),
    yaxis2=dict(
        overlaying="y", side="right",
        tickfont=dict(size=11), tickprefix="$",
        title="AOV", title_font=dict(size=11),
        showgrid=False,
    ),
    hovermode="x unified",
)
...
_ai_chart_insight(
    key="overview_cr_aov",
    context=(
        f"Dual-axis line chart showing conversion rate and AOV over the last {days} days. "
...
st.markdown(
    f'<div class="section-header">Top {top_n} products · last {days} days</div>',
    unsafe_allow_html=True,
)
...
if top_prod.empty:
    ...
    top_prod_display["revenue"] = top_prod_display["revenue"].apply(
        lambda x: f"${x:,.0f}"
    )
    top_prod_display["units_sold"] = top_prod_display["units_sold"].apply(
        lambda x: f"{x:,.0f}"
    )
...
_ai_chart_insight(
    ...
    context=(
        f"Table of top {top_n} products by revenue over the last {days} days. "
