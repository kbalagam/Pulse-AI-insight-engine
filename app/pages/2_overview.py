...
from analytics.metrics import (
    load_daily_metrics, load_product_sales, enrich_daily_metrics,
    compute_period_summary, compute_top_products,
    load_channel_metrics, compute_channel_summary,
)
                ...
                try:
                    ...
                    r = requests.post(
                        ...
                        json={"contents": [{"parts": [{"text": (
                            "You are a business analyst. Give 2-3 concise, specific, actionable "
...
top_prod    = compute_top_products(prod, n=top_n, days=days, start_date=start_date, end_date=end_date)
...
if has_key:
    sum_key = f"overview_summary_{start_date}_{end_date}"
    ...
    if narrative:
        st.markdown(f'<div class="ai-strip"><div class="ai-strip-label">AI summary</div>{narrative}</div>',
                    unsafe_allow_html=True)
else:
    st.markdown('<div class="ai-placeholder">Add a Gemini API key in the sidebar to unlock the AI page summary.</div>',
                unsafe_allow_html=True)
...
with k8:
    ...
    _kpi_card("Anomalies detected", str(n_anom),
              f"{start_date.strftime('%b %d')} – {end_date.strftime('%b %d')}", "kpi-delta-neu",
...
with ch_left:
    ...
    st.markdown(
        f'<div class="chart-sub">{start_date.strftime("%b %d, %Y")} — {end_date.strftime("%b %d, %Y")} · 7-day rolling average</div>',
        unsafe_allow_html=True)
    fig = go.Figure()
    ...
    _ai_chart_insight("overview_revenue",
                      f"Daily revenue bar chart from {start_date} to {end_date}. "
                      f"Recent avg: {_fmt_currency(rev.get('recent_avg'))}, "
                      f"prior avg: {_fmt_currency(rev.get('prior_avg'))}, "
                      f"change: {_fmt_pct(rev.get('pct_change'))}.",
                      ai_on, has_key)

with ch_right:
    st.markdown('<div class="chart-title">Anomalies detected</div>', unsafe_allow_html=True)
    st.markdown(
        f'<div class="chart-sub">{start_date.strftime("%b %d")} – {end_date.strftime("%b %d")} · all metrics</div>',
        unsafe_allow_html=True)
    if recent_anom.empty:
        st.markdown("<div style='padding:1rem 0; color:#9CA3AF; font-size:0.8rem;'>"
                    "No anomalies in this window.</div>", unsafe_allow_html=True)
    else:
...
st.markdown(
    f'<div class="chart-sub">{start_date.strftime("%b %d, %Y")} — {end_date.strftime("%b %d, %Y")} · dual axis</div>',
    unsafe_allow_html=True)
...
_ai_chart_insight("overview_cr_aov",
                  f"Dual-axis chart from {start_date} to {end_date}. "
                  f"Conv rate avg: {cr.get('recent_avg', 0)*100:.2f}%, "
                  f"AOV avg: {_fmt_currency(aov.get('recent_avg'))}.",
                  ai_on, has_key)

# ── Top products ──────────────────────────────────────────────────────────────

st.markdown(
    f'<div class="section-header">Top {top_n} products · {start_date.strftime("%b %d")} – {end_date.strftime("%b %d, %Y")}</div>',
    unsafe_allow_html=True)
...
_ai_chart_insight("overview_products",
                  f"Top {top_n} products from {start_date} to {end_date}. "
                  f"Top: {top_prod.iloc[0]['product_label'] if not top_prod.empty else 'N/A'} "
                  f"at {_fmt_currency(top_prod.iloc[0]['revenue']) if not top_prod.empty else 'N/A'}.",
                  ai_on, has_key)
