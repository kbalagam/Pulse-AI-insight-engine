            ...
            with st.spinner("Analysing..."):
                try:
                    ...
                    prompt = (
                        ...
                        "plain English. No bullet points. Do not start with 'I'.\n\n"
                        f"Context: {context}"
                    )
                    url = (
...
with k1:
    _kpi_card("Last actual (daily)", _fmt_currency(last_actual),
              hint="Most recent data point")
with k2:
    _kpi_card("30-day actual avg", _fmt_currency(avg_actual_30),
              hint="Recent baseline")
with k3:
    _kpi_card(f"Avg forecast ({horizon}d)", _fmt_currency(avg_forecast),
              delta_str=_fmt_pct(fc_change_pct), delta_class=delta_cls,
              hint="vs 30-day actual avg")
with k4:
    _kpi_card("95% CI range (daily)",
...
st.markdown(f'<div class="chart-title">Revenue forecast · {horizon}-day horizon</div>',
            unsafe_allow_html=True)
st.markdown(
    '<div class="chart-sub">Actual (solid blue) · Forecast (dashed) · 95% confidence band (shaded)</div>',
    unsafe_allow_html=True)
...
fig.add_trace(go.Scatter(
    ...
    fill="toself", fillcolor="rgba(29, 78, 216, 0.08)",
    line=dict(color="rgba(0,0,0,0)"), name="95% CI", hoverinfo="skip",
))
fig.add_trace(go.Scatter(
    x=hist_plot["date"], y=hist_plot["actual"].round(0), name="Actual",
    line=dict(color="#1D4ED8", width=2),
    hovertemplate="<b>%{x|%b %d}</b><br>Actual: $%{y:,.0f}<extra></extra>",
))
fig.add_trace(go.Scatter(
    x=hist_plot["date"], y=hist_plot["forecast"].round(0), name="Model fit",
    line=dict(color="#93C5FD", width=1.5, dash="dot"),
    hovertemplate="<b>%{x|%b %d}</b><br>Fitted: $%{y:,.0f}<extra></extra>",
))
fig.add_trace(go.Scatter(
    x=future["date"], y=future["forecast"].round(0), name="Forecast",
...
if not hist.empty and not future.empty:
    fig.add_vline(
        ...
        line_dash="dot", line_color="#E5E7EB", line_width=1.5,
    )
fig.update_layout(
...
_ai_chart_insight(
    key="forecast_main",
    context=(f"Revenue forecast chart showing last 90 days of actual data and "
             f"{horizon}-day forward forecast using Holt-Winters. "
             f"Last actual: {_fmt_currency(last_actual)}. "
             f"Avg forecast: {_fmt_currency(avg_forecast)} ({_fmt_pct(fc_change_pct)} vs 30d avg). "
             f"95% CI: {_fmt_currency(lower_avg)} to {_fmt_currency(upper_avg)} per day."),
    ai_on=ai_on, has_key=has_key,
)

# ── Weekly seasonality charts ─────────────────────────────────────────────────

st.markdown("<div style='height:0.4rem'></div>", unsafe_allow_html=True)
seas_l, seas_r = st.columns(2, gap="small")

with seas_l:
    st.markdown('<div class="chart-title">Average revenue by day of week</div>',
                unsafe_allow_html=True)
    st.markdown(
    ...
    dow_avg          = hist_copy.groupby("dow")["actual"].mean().reindex(dow_order).round(0)
    overall_avg      = dow_avg.mean()
    bar_colors       = ["#1D4ED8" if v >= overall_avg else "#BFDBFE" for v in dow_avg.values]
    fig2 = go.Figure(go.Bar(
        x=dow_avg.index, y=dow_avg.values, marker_color=bar_colors,
        hovertemplate="<b>%{x}</b><br>Avg revenue: $%{y:,.0f}<extra></extra>",
    ))
    fig2.add_hline(
        y=overall_avg, line_dash="dash", line_color="#E5E7EB", line_width=1.5,
    ...
    _ai_chart_insight(
        key="forecast_dow",
        context=(f"Bar chart of average daily revenue by day of week. "
                 f"Overall daily avg: ${overall_avg:,.0f}. "
...
with seas_r:
    st.markdown('<div class="chart-title">30-day rolling average revenue</div>',
                unsafe_allow_html=True)
    st.markdown('<div class="chart-sub">Smoothed trend · all historical data</div>',
                unsafe_allow_html=True)
    roll30           = hist.copy()
    roll30["roll30"] = roll30["actual"].rolling(window=30, min_periods=1).mean().round(0)
    fig3 = go.Figure()
    fig3.add_trace(go.Scatter(
        x=roll30["date"], y=roll30["actual"].round(0), name="Daily actual",
        line=dict(color="#BFDBFE", width=1),
        hovertemplate="<b>%{x|%b %d}</b><br>Actual: $%{y:,.0f}<extra></extra>",
    ))
    fig3.add_trace(go.Scatter(
        x=roll30["date"], y=roll30["roll30"], name="30-day avg",
        line=dict(color="#1D4ED8", width=2),
    ...
    _ai_chart_insight(
        ...
        context=(f"Line chart showing daily actual revenue and 30-day rolling average. "
